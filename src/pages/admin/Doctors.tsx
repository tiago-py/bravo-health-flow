...
const AdminDoctors = () => {
  ...
  const createDoctor = async (doctorData: any) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Erro ao criar médico: ${response.status}`);
      }
  ...
  const updateDoctor = async (id: string, doctorData: any) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Erro ao atualizar médico: ${response.status}`);
      }
      
      const updatedDoctor = await response.json();
      return updatedDoctor;
    } catch (err) {
      throw new Error(err instanceof Error ? err.message : 'Erro ao atualizar médico');
    }
  };
  ...
  const deleteDoctor = async (id: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Erro ao deletar médico: ${response.status}`);
      }
    } catch (err) {
      throw new Error(err instanceof Error ? err.message : 'Erro ao deletar médico');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await fetchDoctors(); // Recarregar a lista
      resetForm();
    } catch (err) {
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-500">
                      ...
                      <div className="flex items-center">
                        ...
                        {doctor.patientsCount || 0} pacientes atribuídos
                      </div>
                    </div>
                  </div>
